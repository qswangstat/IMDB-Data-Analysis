...
hist_plot(movie$imdb_score, bwidth = 0.1, 
          ...
          color = "black") 
```
- There exists right-skewness in the distribution of the IMDB score, most of which lie around $6.7$. We might take $\log$ transformation to handle this problem.
...
## Split the Dataset
Notice that if we try to decide whether to invest a movie or not, some variables cannot be obtained in advance, such as **num_user_for_reviews**, **num_voted_users**, etc. Besides, some text varaibles have no contribution. So, we simply drop them in our analysis.
```{r}
...
movie = movie[, idx]
movie[, 'color'] = as.factor(as.data.frame(movie[, 'color']))
```
